...
int main(int argc, char *argv[]) {
	...
	dst_fd = open(dst_name, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
	...
	const size_t page_size = dst_stat.st_blksize > 0 ? dst_stat.st_blksize : BUFSIZ;
	...
	if (len > 0) {
		...
		if (src == NULL || posix_madvise((void*)src, len, POSIX_MADV_SEQUENTIAL) != 0) {
			perror(src_name);
			return EX_UNAVAILABLE;
		}

		dst = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, dst_fd, 0);
		if (dst == NULL || posix_madvise(dst, len, POSIX_MADV_SEQUENTIAL) != 0) {
			perror(dst_name);
			return EX_UNAVAILABLE;
		}
